...
import { Cliente } from '../../clientes/ClientesManager'
    ...
    return (
        <div className="space-y-6 max-w-4xl">
            ...
            <section className="bg-[#0B0314]/60 backdrop-blur-xl border border-white/5 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            autoComplete="off"
                            className="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700 rounded-xl text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-fuchsia-500 focus:border-transparent transition-all peer"
                        />
                        {/* Selector de clientes flotante */}
                        {clientes.length > 0 && clienteNombre.length > 0 && (
                            <div className="absolute top-full left-0 right-0 mt-1 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden z-20 invisible opacity-0 peer-focus:visible peer-focus:opacity-100 hover:visible hover:opacity-100 transition-all max-h-48 overflow-y-auto">
                                {clientes
                                    .filter(c => c.nombre.toLowerCase().includes(clienteNombre.toLowerCase()) ||
                                        (c.empresa && c.empresa.toLowerCase().includes(clienteNombre.toLowerCase())))
                                    .map(cliente => (
                                        <div
                                            key={cliente.id}
                                            onMouseDown={(e) => {
                                                ...
                                                // Opcional: Podrías guardar el ID del cliente seleccionado si quisieras vincularlo relacionalmente,
                                                // por ahora solo autocompletamos los campos de texto como estaba originalmente.
                                            }}
                                        ...
                                        >
                                            ...
                                            {cliente.empresa && <div className="text-xs text-slate-400">{cliente.empresa}</div>}
                                        </div>
                                    ))}
                            </div>
                        )}
                ...
                <div className="mt-4 pt-4 border-t border-white/5">
                    ...
                    <select
                        ...
                        className="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-500 transition-all appearance-none"
                    >
            ...
            <section className="bg-[#0B0314]/60 backdrop-blur-xl border border-white/5 rounded-2xl p-6 space-y-5">
                ...
                {/* Hoja de Anexos */}
                <ToggleSection
                    label="Hoja de Anexos (Textos largos o explicativos)"
                    enabled={showAnexos}
                    onToggle={() => setShowAnexos(!showAnexos)}
                >
                    <div className="relative">
                        <textarea
                            value={anexos}
                            ...
                            rows={6}
                            placeholder="Escribe aquí descripciones detalladas, alcances del proyecto o cualquier otra información extensa que se añadirá como una hoja extra al final del documento..."
                            className="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700 rounded-xl text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-fuchsia-500 focus:border-transparent transition-all resize-none pr-12"
                        />
                        <div className="absolute top-2 right-2">
                            <AIWriterButton currentText={anexos} context="Anexos, detalles técnicos, etapas del proyecto o alcances detallados" onUpdate={setAnexos} />
                        </div>
                    </div>
                </ToggleSection>
            </section>
